...
public class MutantPillar : ModProjectile
{
	...
	public override void Kill(int timeLeft)
	{
		...
		if (FargoSoulsUtil.BossIsAlive(ref CSENpcs.RealMutantEX, ModContent.NPCType<RealMutantEX>()) && Main.npc[CSENpcs.RealMutantEX].ai[0] == 19f)
		{
			fragmentDuration = (int)Main.npc[CSENpcs.RealMutantEX].localAI[0];
		}
		float speed = 5.5f;
		for (int j = 0; j < 4; j++)
		{
